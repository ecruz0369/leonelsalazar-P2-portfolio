...
```{r}
ggplot(rawdata,aes(`Age`,Height))+geom_boxplot()

ggplot(rawdata,aes(`HS Graduate`,Height))+geom_boxplot()
```
...
```{r}
ggplot(rawdata,aes(x = `HS Graduate`, y = Height)) +
  geom_point()

ggplot(rawdata,aes(x = `Age`, y = Height)) +
  geom_point()
```
...
```{r}
p1 <- rawdata %>% ggplot(aes(x=Age)) + geom_histogram() 
plot(p1)
